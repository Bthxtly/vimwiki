...
class ProcedureDecl(AST):
    def __init__(self, proc_name, block_node) -> None:
...
class Var(AST):  # The Var node is constructed out of ID token
    def __init__(self, token) -> None:
        self.token = token
        self.value = token.value
...
class SemanticAnalyzer(NodeVisitor):
    ...
    def visit_Assign(self, node):
        ...
        if var_symbol is None:
            raise Exception(f"Error: Identifier not found '{var_name}'")
        self.visit(node.right)
    ...
    def visit_ProcedureDecl(self, _):
        pass
...
def print_AST(ast, prefix="", next=False):
    ...
    if astype == "Compound":
        ...
        for node in ast.declarations:
            ...
            elif type(node).__name__ == "ProcedureDecl":
                print(prefix + "├── " + "procedure " + node.proc_name)
                print_AST(node.block_node, prefix + "│   ")
    ...
    elif astype == "Var" or astype == "Num":
        branch = "├── " if next else "└── "
        print(prefix + branch + str(ast.value))

    elif astype == "Program":
        print("program " + ast.name)
        print_AST(ast.block)
